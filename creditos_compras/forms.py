...
class CompraCreditoForm(forms.ModelForm):
    ...
    class Meta:
        model = Purchase
        fields = ['supplier', 'document_number', 'subtotal', 'tax', 'total', 'tipo_pago']
        widgets = {
...
class GenerarCuotasCompraForm(forms.Form):
    numero_cuotas = forms.IntegerField(
        min_value=1,
        label='Número de cuotas',
        widget=forms.NumberInput(attrs={'class': 'form-control', 'min': 1}),
...
class PagoMultipleCuotasCompraForm(forms.Form):
